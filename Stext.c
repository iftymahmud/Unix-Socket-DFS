...
#define TEXT_DIR "/home/mahmud74/stext"
...
int main() {
    ...
    // Create the base directory for TEXT files
    mkdir(TEXT_DIR, 0755);
    ...
    printf("Stext server is running...\n");
...
void create_tar_and_send(int smain_socket) {
    ...
    snprintf(tar_file, BUFFER_SIZE, "/tmp/txt_files_%d.tar", getpid());

    // Command to find all .txt files in TEXT_DIR and create a tarball
    snprintf(command, BUFFER_SIZE, "find %s -type f -name '*.txt' -print0 | tar --null -cvf %s --files-from - 2>/dev/null", TEXT_DIR, tar_file);
...
void list_files(int smain_socket, char *path) {
    ...
    // If the path starts with "/home/mahmud74/smain", replace it with "/home/mahmud74/stext"
    if (strncmp(path, "/home/mahmud74/smain", 20) == 0) {
        snprintf(command, BUFFER_SIZE, "find %s%s -type f -name '*.txt'", TEXT_DIR, path + 20);
    } else {
        // Otherwise, treat it as a relative path under TEXT_DIR
        snprintf(command, BUFFER_SIZE, "find %s/%s -type f -name '*.txt'", TEXT_DIR, path);
    }
    ...
    while (fgets(buffer, BUFFER_SIZE, fp) != NULL) {
        ...
        // printf("Found file: %s", buffer);  // Debugging line
